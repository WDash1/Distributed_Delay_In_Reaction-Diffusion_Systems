...
tau_amt = 3;
tau_values = linspace(0.01,0.1,tau_amt);
...
dydt = @(t,y,Z) computeFixedDelayRLBDerivative(a,b,Du,Dv,m,h,y,Z);
...
parfor tau_index = 1:tau_amt
    ...
    colorbar
    filename1 = 'Output_Images/RLB_Fixed_Delay_PDE_Model_u_values_tau='+string(tau)+'a='+string(a)+'_b='+string(b)+'_m='+string(m)+'_t='+max(t_mesh);
    ...
    set(gca,'yticklabel',num2str(get(gca,'ytick')','%.0f'))
    colorbar
    filename2 = 'Output_Images/RLB_Fixed_Delay_PDE_Model_v_values_tau='+string(tau)+'a='+string(a)+'_b='+string(b)+'_m='+string(m)+'_t='+max(t_mesh);
